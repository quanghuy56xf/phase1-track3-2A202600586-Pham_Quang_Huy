...
from .utils import format_reflection_memory, normalize_answer
...
class BaseAgent:
    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            reset_call_metrics()
            answer = actor_answer(
                example, attempt_id, self.agent_type, reflection_memory, previous_answers
            )
            ...
            if self._is_looping(answer, previous_answers):
                traces.append(trace)
            ...
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts:
                ...
                reflection_memory.append(format_reflection_memory(reflection, answer, attempt_id))
                trace.reflection = reflection
            previous_answers.append(answer)
            traces.append(trace)
        if judge is not None and failure_mode not in ("none", "looping"):
    ...
    @staticmethod
    def _is_looping(answer: str, previous_answers: list[str]) -> bool:
        norm = normalize_answer(answer)
        return any(norm == normalize_answer(prev) for prev in previous_answers)
